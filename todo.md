...
> **Status:** **Phase 1 (Microkernel POC) in pieno corso** — Phase 0 chiusa (P0/P1/P2 ✅; P3/P4 parziali per dipendenze esterne — funding/cryptographer). Track A desktop ✅ (M1-M5 + M3b). Track B kernel ✅ **MB1-MB13** chiusi sul branch `feat/kernel-mb11-userspace` (post-v0.2.0). MB14.a + MB14.b + MB14.c.1 + MB14.c.2.a + MB14.c.2.b.1 + MB14.c.2.b.2 + MB14.c.2.c + MB14.c.2.d + MB14.d + MB14.e + MB14.f + **MB14.g** (per-CPU plumbing: `PerCpu` esteso con `tick_count` + `need_resched` atomici per-CPU; LAPIC timer ISR scrive solo `current_cpu()` storage — drop dell'is_bsp gate MB14.f; `crate::TICK_COUNT` global rimosso; `RoundRobinScheduler::enqueue_for_cpu`/`pick_next_for_cpu` dual-write/read tra `per_cpu_run_queue::*` (bare-metal) e i `run_queues` locali (host/test); smoke `[mb14.g] per_cpu tick=N resched=ok sched_route=ok`; +7 host test) ✅ chiusi 2026-05-20. Roadmap Phase 1 ~95% Track B. **Prossimo sub-block: MB14.h** = AP-side dispatch loop attivo (AP esegue scheduler con `pick_next_for_cpu(cpu_id)` consumando dalla propria per_cpu_run_queue, context-switch cross-CPU safety review, IST stack sharing rules, ADR-0009), poi PR su `main`.
...
| MB14.g | Per-CPU plumbing: `PerCpu` esteso con `tick_count` + `need_resched` atomici per-CPU; LAPIC timer ISR scrive solo `current_cpu()`; `crate::TICK_COUNT` global rimosso; `RoundRobinScheduler::enqueue_for_cpu`/`pick_next_for_cpu` dual-write/read tra `per_cpu_run_queue` e legacy mirror; smoke `[mb14.g]` + 7 host test | `[x]` (chiuso 2026-05-20) | (this commit) | — |
| MB14 | MP/AP enable + TLB shootdown cross-AS (Phase 1.5) | `[~]` (MB14.a + MB14.b + MB14.c.* + MB14.d + MB14.e + MB14.f + MB14.g chiusi; MB14.h open) | — | — |
