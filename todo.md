...
- [~] **P6.7.0 — `OIP-Driver-Framework-013` drafting** (`[Draft]` 2026-05-20) — `oips/oip-driver-framework-013.md`. Lock kernel-side contract per 5 superfici: capability scope extensions (`Action::{MmioMap, DmaMap, IrqAttach, PciConfigRead, PciConfigWrite, DriverLoad, DriverUnload}` + `Resource::{PciDevice, MmioRegion, DmaWindow, IrqLine}`), MMIO syscall `SyscallNo::MmioMap = 22`, DMA/IOMMU domain-per-driver con backend `vtd.rs`+`amdvi.rs`, IRQ routing BSP-only con shared-line rejection (`EBUSY`), driver manifest TOML + Ed25519 signing + static `KNOWN_ISSUERS`.
- [ ] **P6.7.1 — Promote `OIP-Driver-Framework-013` Draft → Review** — `oip-lint` already green; editorial pass on RFC 2119 wording + § R6 "not doing" list completeness; transition under `OIP-Process-001` § 4.
- [ ] **P6.7.2 — Promote `OIP-Driver-Framework-013` Review → Last Call** — 14-day public-objection window per `OIP-Process-001` § 5.3.
- [ ] **P6.7.3 — Framework impl skeleton** (no driver code yet) — `crates/omni-capability/src/scope.rs` new variants + tests, `crates/omni-kernel/src/bare_metal/syscall_entry.rs` new syscall numbers + handler stubs returning `ENOSYS`, `crates/omni-kernel/src/driver_manifest.rs` new module + TOML parsing + signature verification, `crates/omni-kernel/src/known_issuers.rs` static table generated from `docs/protocol/driver-issuers.toml`. Acceptance: `cargo test --workspace --all-features` + clippy bare-metal + clippy mb12-userprobe clean.
- [ ] **P6.7.4 — `OIP-Driver-NVMe-XXX` drafting** — file under `oips/oip-driver-nvme-XXX.md` (next free integer at filing); inherit `requires: [13]`; lock NVMe-specific manifest fields (admin/IO submission queue depth, PRP/SGL choice), command channel ABI (`omni.driver.nvme.cmd`/`evt`/`irq`), and BLK-layer integration with the future file-system service.
- [ ] **P6.7.5 — `OIP-Driver-Net-XXX` drafting** — virtio-net (host) + Intel e1000e (bare-metal) + Mellanox ConnectX (server). Phased delivery: virtio-net first (QEMU/Proxmox bring-up), e1000e second (bare-metal demo).
- [ ] **P6.7.6 — `OIP-Driver-TEE-XXX` drafting** — promote stub TEE backend to a real driver via `IpcCreateChannel` + DMA windows for attestation report flow. Replaces `StubAttestation` from MB13.c.
