...
> **Status:** **Phase 1 (Microkernel POC) in pieno corso** — Phase 0 chiusa (P0/P1/P2 ✅; P3/P4 parziali per dipendenze esterne — funding/cryptographer). Track A desktop ✅ (M1-M5 + M3b). Track B kernel ✅ **MB1-MB13** chiusi sul branch `feat/kernel-mb11-userspace` (post-v0.2.0). MB14.a + MB14.b + MB14.c.1 + MB14.c.2.a + MB14.c.2.b.1 + MB14.c.2.b.2 + MB14.c.2.c + **MB14.c.2.d** (real per-AP init: per-CPU slot array + per-AP kstack/IST/TSS + GDT estesa a 69 slot + `ApRuntimeControl` block + nuovo `kmain_ap` asm che fa lgdt/lidt/wrmsr GS_BASE/ltr/park) ✅ chiusi 2026-05-19. Roadmap Phase 1 ~90% Track B. **Prossimo sub-block: MB14.d** = TLB shootdown IPI broadcast (vettore `0xFD` + `ipi::send_to_all_except_self` + `mm::flush_tlb_range`); poi MB14.e (per-CPU run-queue + scheduler split), MB14.f (x2APIC LAPIC ID > 255).
...
| MB14.c.2.d | Real per-AP init (AP_SLOTS + per-AP kstack/IST/TSS + GDT extended to 69 slots + ApRuntimeControl + real `kmain_ap` asm: lgdt/lidt/wrmsr GS_BASE/ltr/park) | `[x]` (chiuso 2026-05-19) | (this commit) | — |
| MB14 | MP/AP enable + TLB shootdown cross-AS (Phase 1.5) | `[~]` (MB14.a + MB14.b + MB14.c.* chiusi; MB14.d-f open) | — | — |
