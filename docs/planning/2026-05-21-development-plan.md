...
| R12 | `docs/02-architecture.md` line 251 still lists "Filesystem: native OMNI FS vs. existing options (ZFS port, ext4 via compatibility)" as an open architectural question. Resolving it shapes TASK-011 substantially. | Medium | Medium | **Open question for the lead:** decide the filesystem direction before TASK-011 enters Wave 4, OR explicitly mark TASK-011 as a stub-only crate scaffold (current default). |
