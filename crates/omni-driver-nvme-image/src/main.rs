//! OMNI OS NVMe bootable driver image — P6.7.8.5.
//!
...
//! follow-up P6.7.8.x).
//!
//! This image is the smallest possible *runnable* NVMe driver: it walks
//! the [`omni_driver_nvme::bringup::BringUp`] FSM from
//! `Phase::PciEnumeration` → `Phase::Ready`, posting `Event::Advance`
//! on each step. The actual syscall invocations are stubbed for
//! P6.7.8.5 (the real `MmioMap` / `DmaMap` / `IrqAttach` arg layouts
...
//! two-register path.
//!
//! Pattern mirrors the `omni-driver-net-virtio-image` sibling
//! introduced in P6.7.8.3 and the `omni-kernel` ↔ `kernel-runner`
//! split that already powers the bare-metal boot path.
//!
//! Build:
//!
//! ```sh
...
//! ```
//!
//! The resulting `target/x86_64-unknown-none/release/omni-driver-nvme-image`
...
// =============================================================================
//
// The `omni-driver-nvme` library has `extern crate alloc` because future
// bring-up code will need `alloc::vec::Vec` for the Identify response
// parsers and the per-namespace BLK request table. The P6.7.8.5
// skeleton does not allocate at runtime — the bring-up FSM is `Copy`,
// the syscall wrappers use `[u64; 6]` on the stack, and we never
// instantiate any heap container.
//
...
//     (the bring-up should not need `alloc` until the Identify
//     response parsing lands), and
...
unsafe impl GlobalAlloc for PanicOnAlloc {
    unsafe fn alloc(&self, _layout: Layout) -> *mut u8 {
        ...
        panic!(
            "omni-driver-nvme-image: heap alloc requested but no allocator is wired (P6.7.8.x follow-up)"
        );
    }
