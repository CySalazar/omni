//! OMNI OS virtio-net bootable driver image — P6.7.8.3.
//!
...
//! follow-up P6.7.8.x).
//!
//! This image is the smallest possible *runnable* driver: it walks the
//! [`omni_driver_net_virtio::bringup::BringUp`] FSM from
//! `Phase::Reset` → `Phase::DriverOk`, posting `Event::Advance` on each
//! step. The actual syscall invocations are stubbed for P6.7.8.3 (the
//! real `MmioMap` / `DmaMap` / `IrqAttach` arg layouts depend on the
//! `DriverLoad` capability deposit, which is a follow-up); they appear
//! as raw `syscall` instructions with `rax = SyscallNumber` and the
//! kernel returns through the rich two-register path.
//!
//! Build:
//!
//! ```sh
...
// table. The P6.7.8.3 skeleton does not allocate at runtime — the
// bring-up FSM is `Copy`, the syscall wrappers use `[u64; 6]` on the
// stack, and we never instantiate any heap container.
//
...
//     (the bring-up should not need `alloc` until the RX pool lands),
//   - and signals the missing functionality loudly when that
//     bug appears (via `TaskExit(2)` from the panic handler below).
...
unsafe impl GlobalAlloc for PanicOnAlloc {
    unsafe fn alloc(&self, _layout: Layout) -> *mut u8 {
        ...
        panic!("omni-driver-net-virtio-image: heap alloc requested but no allocator is wired (P6.7.8.x follow-up)");
    }
...
#[allow(dead_code, reason = "wired in P6.7.8.x once DriverLoad deposits the tokens")]
#[inline(always)]
unsafe fn syscall5(
    number: u64,
    a0: u64,
...
pub extern "C" fn _start() -> ! {
    // P6.7.8.3 driver-image skeleton:
    //
    // 1. Drive the bring-up FSM to DriverOk in a self-test loop. The
    //    actual MmioMap / DmaMap / IrqAttach syscalls are stubbed for
    //    P6.7.8.3 — they require the capability tokens the kernel
    ...
    // Exit with 0 when the FSM converged to DriverOk; non-zero on any
